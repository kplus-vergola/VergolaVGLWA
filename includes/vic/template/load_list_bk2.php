...
{	
    ...
    $sql = "INSERT INTO ver_chronoforms_data_letters_vic (clientid, template_name, datecreated, template_content, template_type) 
		 VALUES ('$template_clientid','$template_title', '$current_date', '$template_content', 'check list')"; 
	mysql_query($sql); 
...
<div style=" font-size: 13pt; font-weight: bold; padding:10px; width: 85%; text-align: center; margin: 0 auto;">Vergola Installer General Load List</div>

<div style="margin: 5px 0 5px 50px;"><b>Job No.: <?php  echo $contract["projectid"]; ?></b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;  <b>Description: <?php  echo $contract["framework"]; ?></b></div> 

<div style="border:1px solid; margin: 0 3% 0 3%; padding:8px; width: 43%; display: inline-block; vertical-align: top; ">
		...
		<p style='width:150px;display: inline-block; border: 0px solid; margin: 5px 0 5px 0;'>Wk Ph:<?php echo $contract["client_wkphone"]; ?></p><p style='width:100px;display: inline-block; border: 0px solid;' >Hm Ph:<?php echo $contract["client_hmphone"]; ?></p> 
		<span>Mobile:</span><?php echo $contract["client_mobile"]; ?> 
</div>
<div style="border:1px solid; solid; margin: 0 0% 0 3%; padding:5px; width: 43%;   display: inline-block; min-height: 62px;">  
		<p><b>Installer:</b> <?php echo $contract["erectors_name"]; ?> </p>
		<p style='width:150px;display: inline-block; border: 0px solid; margin: 5px 0 5px 0;'>Fees: $<?php echo $contract["client_wkphone"]; ?></p><p style='width:100px;display: inline-block; border: 0px solid;' >Extras: $<?php echo $contract["client_hmphone"]; ?></p> 
		...
		$has_materials = 0;
			...
			$titleID = $ListProjectID."_"."frame"."_".mt_rand(); 
					...
					WHERE bm.projectid = '{$projectid}' AND inv.section='{$cat['section']}' ".($is_reorder==1?" AND bm.inventoryid='{$inventoryid}' ":"  ")."  AND m.is_main_item=1  "; //ORDER BY is_per_length DESC, bm.id, b.length DESC, b.qty DESC
 					...
 					$sql2 = "SELECT bm.id,   m.qty, (m.qty*bm.qty) AS m_qty, bm.length, bm.qty  AS ls_qty, 
					    CASE WHEN m.uom='Mtrs' 
					        THEN bm.raw_cost * bm.qty * bm.length  
					        ELSE bm.raw_cost * bm.qty END AS ls_amount,  
					...
					JOIN ver_chronoforms_data_inventory_vic AS inv ON inv.inventoryid=bm.inventoryid 				
					JOIN ver_chronoforms_data_materials_vic AS m ON m.cf_id=bm.materialid  
					WHERE bm.projectid = '{$projectid}' AND inv.section='{$cat['section']}'".($is_reorder==1?" AND bm.inventoryid='{$inventoryid}' ":"  ")."   AND m.is_main_item=0   ";//ORDER BY is_per_length DESC
