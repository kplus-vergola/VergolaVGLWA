...
if(isset($VergolaType) && ($VergolaType=="Four Bay VR6" )){
	$projectId = $ProjectID;
}else{
	$projectId = null;
} 
...
function generateRowItem2($section,$projectId=null,$VergolaType="Four Bay VR6"){
	$tr = "";
	...
	if($section=="frame"){  
		...
		while ($r = mysql_fetch_assoc($fw)) { 
	 	...
	 	//error_log("SELECT  i.inventoryid, q.* FROM ver_chronoforms_data_quote_vic AS q LEFT JOIN ver_chronoforms_data_inventory_vic AS i ON i.inventoryid=q.inventoryid WHERE projectid='{$projectId}' AND LOWER(i.section) = 'fixings' ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log'); 
		while ($r = mysql_fetch_assoc($fw)) { 
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  ver_chronoforms_data_inventory_vic AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					}else if($item["inventoryid"]=="IRV44"){  
					...
					}else if(($item["inventoryid"]=="IRV43" || $item["inventoryid"]=="IRV46" )   || $item["inventoryid"]=="IRV47" ){ //error_log("2nd", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
						$input_id="";
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr class=\"tr-vergola ".($isFirst?'trv-1':'').($isSecond?'trv-2':'').($isSecond2?'trv-3':'').($isSecond3?'trv-4':'')." ".($r['is_additional']==1?"tr-added-item":"")." \">"; 
