<?php  
//error_log("inside VR2", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
//error_log(print_r($retrievec,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
if((isset($_POST['update']) || isset($_POST['save_duplicate']) || isset($_POST['save-close']) || isset($_POST['save_close_duplicate'])) && isset($_POST['projectid']))
{
	...
	// error_log($is_create_duplicate, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
function generateRowItem2($section,$projectId=null,$VergolaType="Double Bay VR2"){
	$tr = "";
	 
	//global $VergolaType;
	//error_log("Inside VergolaType: ".$VergolaType, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');

	if($section=="frame"){  
		
		$filter_framework = "";
	 	if($VergolaType == "Double Bay VR2"){
	 		$filter_framework = $VergolaType;
	 	}else if($VergolaType == "Double Bay VR3"){
	 		$filter_framework = "Double Bay VR2"; //use vr2 item bec. they just the same. just the louvers are different side that will differ in length
	 	}else if($VergolaType == "Double Bay VR3 - Gutter"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					//error_log($item["section"], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items  WHERE LOWER(framework) = 'Double Bay VR2' AND LOWER(section) = 'fixings'" ); // 
	 	}else{
	 	...
	 	error_log("SELECT  i.inventoryid, q.* FROM ver_chronoforms_data_quote_vic AS q LEFT JOIN ver_chronoforms_data_inventory_vic AS i ON i.inventoryid=q.inventoryid WHERE projectid='{$projectId}' AND LOWER(i.section) = 'fixings' ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
		while ($r = mysql_fetch_assoc($fw)) { 
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  ver_chronoforms_data_inventory_vic AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'guttering'" ); // 
	 	}else{
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					else if($isFourth==1){$lw = "length2"; $isFourth=0; $isFifth=1;  }
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					if($r['is_additional']==1){
						//error_log("is_additional cf_id :" . $item['cf_id'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'downpipe'" ); // 
	 	}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'vergola'" ); // 
	 	}else{
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr class=\"tr-vergola ".($isFirst?'trv-1':'').($isThird?'trv-2':'')." \">"; 
				...
				if($item["inventoryid"] == "IRV61"){
					...
					//$tr .= "<td class=\"td-qty\"><input type=\"text\" class=\"qtylen\" id=\"IRV64_qty\" name=\"qty[]\" value=\"{$r['qty']}\"></td>";  				
				}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'misc'" ); // 
	 	}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'extras'" ); // 
	 	}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'Disbursements'" ); // 
	 	}else{
