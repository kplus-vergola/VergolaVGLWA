...
//error_log(print_r($retrievec,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
...
if((isset($_POST['update']) || isset($_POST['save_duplicate']) || isset($_POST['save-close']) || isset($_POST['save_close_duplicate'])) && isset($_POST['projectid']))
{
	...
	// error_log($is_create_duplicate, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
	// echo $is_create_duplicate;exit();
	...
	//error_log($querym, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
	$i6 = mysql_query($querym) or trigger_error("Insert failed: " . mysql_error());
	    
	$db_result = 0; 
	$note = null;   

	//error_log("i1:".$i1." i2:".$i2." i3:".$i3." i4:".$i4." i5:".$i5." i6:".$i6, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
...
if(isset($VergolaType) && ($VergolaType=="Three Bay VR4" )){
	$projectId = $ProjectID;
}else{
	$projectId = null;
} 
...
function generateRowItem2($section,$projectId=null,$VergolaType="Three Bay VR4"){
	$tr = "";
	 
	//global $VergolaType;
	//error_log("Inside VergolaType: ".$VergolaType, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');

	if($section=="frame"){  
		
		$filter_framework = "";
	 	if($VergolaType == "Three Bay VR4"){
	 		$filter_framework = $VergolaType;
	 	} 
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					if(strtolower($item["section"])=="frame" && (strtolower($item["category"])=="beams" || strtolower($item["category"])=="posts" || strtolower($item["category"])=="beam fixings" ||  strtolower($item["category"])=="intermediate")){ 
						...
						$tr .= "<td class=\"td-item\">".$item["description"]." <input type=\"hidden\" class=\"price\" value=\"{$item['rrp']}\" desc=\"{$item['description']}\" inventoryid=\"{$item['inventoryid']}\" /> </td>";
						$tr .= "<td> <input type=\"hidden\" name=\"webbing[]\" /> </td>";   
						...
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"length input-in\" value=\"{$r['length']}\" style=\"display:none\"></td>";  	
						if(METRIC_SYSTEM=="inch"){
							$tr .= "<td class=\"td-ft\"><input type=\"text\"    class=\"length  input-ft\" value=\"".get_feet_value($r['length'])."\" style=\"display:none\"></td>";  
						}	
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items  WHERE LOWER(framework) = 'Three Bay VR4' AND LOWER(section) = 'fixings'" );  
	 	}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'guttering'" ); // 
	 	}else{
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			//error_log("inventoryid: ".$r["inventoryid"]." length: ".$r["length"]." rrp: ".$r["rrp"], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');

			$tr .= "<tr>"; 
					$lw = ""; 
					if($isFirst==1){$lw = "length"; $isFirst=0; $isSecond=1; }
					else if($isSecond==1){$lw = "length2"; $isSecond=0; $isThird=0; } 
					else if($isSecond2==1){$lw = "length3"; $isSecond2=0; $isThird=1; } 
					else if($isThird==1){$lw = "length"; $isThird=0; $isFourth=1; }
					else if($isFourth==1){$lw = "length2"; $isFourth=0; $isFifth=0;}
					else if($isFourth2==1){$lw = "length3"; $isFourth2=0; $isFifth=1;  }
					...
					if($isSecond == 1){ 
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"{$lw} input-in gutter-length\" value=\"{$r['length']}\"></td>"; 
						if(METRIC_SYSTEM=="inch"){
							$tr .= "<td class=\"td-ft\"><input type=\"text\"  class=\"{$lw} input-ft gutter-length\" value=\"".get_feet_value($r['length'])."\"></td>";
						}
					}else if($isThird == 1){ 
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"{$lw}  input-in gutter-length\" value=\"{$r['length']}\"></td>";
						if(METRIC_SYSTEM=="inch"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					if($r['is_additional']==1){
						//error_log("is_additional cf_id :" . $item['cf_id'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
					...
					}else if(($item["inventoryid"]=="IRV43" || $item["inventoryid"]=="IRV46" )  && $isFirst == 1 ){ 
						$input_id="";
						...
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"length input-in\" value=\"{$r['length']}\" id=\"{$input_id}\"></td>"; 
						if(METRIC_SYSTEM=="inch"){
							$tr .= "<td class=\"td-ft\"><input type=\"text\"   class=\"length input-ft\" value=\"".get_feet_value($r['length'])."\" id=\"{$input_id}_ft\"></td>"; 
						}	
						$isFirst = 0; 

					}else if($item["inventoryid"]=="IRV44"){ //error_log("2nd", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
						 
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"width input-in\" value=\"{$r['length']}\"></td>";
						if(METRIC_SYSTEM=="inch"){
						...
						//error_log("here", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'downpipe'" ); // 
	 	}else{
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
				...
				if(METRIC_SYSTEM=="inch"){
					$tr .= "<td class=\"td-ft\"><input type=\"text\"   class=\"input-in\" value=\"".get_feet_value($r['length'])."\" style=\"display:none\"> </td>";  
				}	
				$tr .= "<td class=\"td-rrp\"><input type=\"text\" class=\"rrp\" readonly=\"readonly\" name=\"rrp[]\" value=\"{$r['rrp']}\"> </td>";  
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'vergola'" ); // 
	 	}else{
	 		$fw = mysql_query("SELECT q.* FROM ver_chronoforms_data_quote_vic AS q LEFT JOIN ver_chronoforms_data_inventory_vic AS i ON i.inventoryid=q.inventoryid WHERE projectid='{$projectId}' AND LOWER(i.section) = 'vergola' " ); // 
	 	}

	 	$isFirst = 1; $isSecond = 0; $isSecond2 = 0; $isThird = 0;$isFourth = 0; $isFifth = 0; $isFifth2 = 0; $isSixth=0; $isSeventh=0; $isEighth=0; $isEighth2=0;
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			if($isFirst==1){ 
				...
				$isFifth=0; $isFifth2=1; $isSixth=1; 
				$colorSelection = listColours("",$r);
				$listColourBondSelection = listColourBond(null,$r,"Vergola");
 			...
 			//error_log("is_additional :".$r["is_additional"], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
			...
			$lengthInput_ft = "";
			if($isFirst || $isSecond || $isSecond2){
				$itemName = listItem("louvres",$r,null,"poly".($isFirst?"1":"2"));
				...
				if(METRIC_SYSTEM=="inch"){
					$lengthInput_ft = "<input type=\"text\"  {$itemLen_ft}   class=\"length\" value=\"".get_feet_value($r['length'])."\"  style=\"display:none\" />";
				}	
			}

			$tr .= "<tr class=\"tr-vergola ".($isFirst?'trv-1':'').($isSecond?'trv-2':'').($isSecond2?'trv-3':'')." ".($r['is_additional']==1?"tr-added-item":"")." \">"; 
				$tr .= "<td class=\"td-item\">{$itemName}<input type=\"hidden\" class=\"price\" value=\"{$item['rrp']}\" category=\"{$item['category']}\" inventoryid=\"{$item['inventoryid']}\" desc=\"{$item['description']}\" length=\"{$r['length']}\" /> </td>";
				$tr .= "<td> </td>"; 
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'misc'" ); // 
	 	}else{
		...
		$tr .= "<tr id=\"misc_last_row\" >";  
	 	...
	 	if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'extras'" );
	 	}else{
		...
		if($projectId == null){
	 		$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'Disbursements'" ); // 
	 	}else{
...
if($VergolaType != "Drop-In"){  
	...
	echo "<tr><td colspan=\"8\" class=\"subheading\">Framework</td></tr>";
 	echo generateRowItem2("frame",$ProjectID,$VergolaType); 
...
//error_log("INSIDE -- b4: ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
// ***************************** Louvres Poly or Square ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Vergola System</td></tr>";
echo generateRowItem2("vergola",$ProjectID); 
//error_log("INSIDE -- after: ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
...
//error_log("INSIDE -- last: ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
