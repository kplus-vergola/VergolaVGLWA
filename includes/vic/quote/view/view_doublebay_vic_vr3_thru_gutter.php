...
function generateRowItemVR3($section,$projectId=null){
	$tr = ""; 
	...
	if($section=="frame"){  
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					if(strtolower($item["section"])=="frame" && (strtolower($item["category"])=="beams" || strtolower($item["category"])=="beam fixings" || strtolower($item["category"])=="posts" || strtolower($item["category"])=="intermediate")){ 
						...
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"length input-in\" value=\"".(METRIC_SYSTEM=="inch"?"157.48":"4")."\" style=\"display:none\"></td>";  	
						if(METRIC_SYSTEM=="inch"){
		...
		$isFirst = 1;$isSecond = 0;$isThird = 0;$isFourth = 0;$isFifth=0; $isSixth=0; $isSeventh=0;//should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
		$_isFirst = 1;
		while ($r = mysql_fetch_assoc($fw)) { 
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  ver_chronoforms_data_inventory_vic AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
			...
			$tr .= "<tr>"; 
					...
					else if($isSixth==1){$lw = "width"; $isSixth=0; $isSeventh=1; }
					else if($isSeventh==1){$lw = "width"; $isSeventh=0;}
					...
					if($item["inventoryid"] == "" && $isSecond==1){ 
						$tr .= "<td class=\"td-len\"><input type=\"text\"  id=\"IRV27_length\" name=\"slength[]\" class=\" input-in gutter-length\" value=\"{$r['length']}\" ></td>"; 
						if(METRIC_SYSTEM=="inch"){
				 			$tr .= "<td class=\"td-ft\"><input type=\"text\"  id=\"IRV27_length_ft\" class=\" input-ft gutter-length\" value=\"".get_feet_value($r['length'])."\" ></td>"; 
						}

					}else if($item["inventoryid"] == "IRV33" || $item["inventoryid"] == "IRV34"){ 
						//$tr .= "<td class=\"td-len\"><input type=\"text\"   name=\"slength[]\" class=\"tapered_gutter_IRV33_34_length\" value=\"0\"></td>";  
						if($_isFirst==1){
							$tr .= "<td class=\"td-len\"><input type=\"text\"   name=\"slength[]\" class=\"length input-in gutter-length\" value=\"{$r['length']}\"></td>";  //class = tapered_gutter_IRV33_34_length
							if(METRIC_SYSTEM=="inch"){
				 				$tr .= "<td class=\"td-ft\"><input type=\"text\"  class=\"length input-ft gutter-length\" value=\"".get_feet_value($r['length'])."\"></td>"; 
							}
							$_isFirst = 0;
						}else{
							$tr .= "<td class=\"td-len\"><input type=\"text\"   name=\"slength[]\" class=\"length2 input-in gutter-length\" value=\"{$r['length']}\"></td>";  //class = tapered_gutter_IRV33_34_length
							if(METRIC_SYSTEM=="inch"){
					...
					}else if($item["inventoryid"] == "IRV35"){ 
						$tr .= "<td class=\"td-len\"><input type=\"text\"    name=\"slength[]\" class=\"length input-in gutter-length\" value=\"{$r['length']}\"></td>";  
						if(METRIC_SYSTEM=="inch"){
				 			$tr .= "<td class=\"td-ft\"><input type=\"text\"   class=\"length input-ft gutter-length\" value=\"".get_feet_value($r['length'])."\"></td>";  
						}

					}else if($item["inventoryid"] == "IRV36"){ 
						$tr .= "<td class=\"td-len\"><input type=\"text\"    name=\"slength[]\" class=\"length2 input-in gutter-length\" value=\"{$r['length']}\"></td>";  
						if(METRIC_SYSTEM=="inch"){
					 		$tr .= "<td class=\"td-ft\"><input type=\"text\"  class=\"length2 input-ft gutter-length\" value=\"".get_feet_value($r['length'])."\"></td>"; 
						}
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					if($r['is_additional']==1){
						...
						if(METRIC_SYSTEM=="inch"){
				 			$tr .= "<td class=\"td-ft\"><input type=\"text\"  name=\"slength[]\" class=\"length input-in\" value=\"".get_feet_value($r['length'])."\"></td>";
						} 
					}
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			if($isFirst || $isThird){
				...
				if(METRIC_SYSTEM=="inch"){
				 		$lengthInput_ft = "<input type=\"text\" {$itemLen_ft}  class=\"  input-ft\" value=\"".get_feet_value($r['length'])."\" />";
				}
			}
			else{
				$itemName = $item["description"];
				$lengthInput = "<input type=\"text\"  {$itemLen} name=\"slength[]\" class=\"length input-in\" value=\"{$r['qty']}\"  style=\"display:none\" />";
				if(METRIC_SYSTEM=="inch"){
			...
			$tr .= "<tr class=\"tr-vergola\">"; 
				...
				if($item["inventoryid"] == "IRV61"){
					...
					$tr .= "<td class=\"td-qty\"><input type=\"text\" {$itemQty} class=\"qtylen input-in\" name=\"qty[]\" value=\"{$r['qty']}\"></td>";
				}
...
if($VergolaType != "Drop-In"){  
	...
	echo generateRowItemVR3("frame",$ProjectID); 
	...
	echo generateRowItemVR3("fixings",$ProjectID); 
...
echo generateRowItemVR3("guttering",$ProjectID); 
 
// ***************************** Cbeam Face Flashing Z al ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Flashing</td></tr>";
 echo generateRowItemVR3("flashings",$ProjectID); 
 

// ***************************** Downpipe Plastic 3m ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Downpipe</td></tr>";
echo generateRowItemVR3("downpipe",$ProjectID); 
...
echo generateRowItemVR3("vergola",$ProjectID); 
...
echo generateRowItemVR3("misc",$ProjectID); 
 
// ***************************** Misc Cost ********************************************//
 
// ***************************** Add Extra 1 ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Extras</td></tr>";
echo generateRowItemVR3("extra",$ProjectID); 
  

// ***************************** Shop Drawings ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Disbursements</td></tr>";
echo generateRowItemVR3("disbursements",$ProjectID); 
