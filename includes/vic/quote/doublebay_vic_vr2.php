...
if(isset($_POST['save-doublebay-vr2']) || isset($_POST['save-close-doublebay-vr2'])){	
	...
	if ($cnt > 0) {
		...
		//error_log("INSERT INTO ver_chronoforms_data_measurement_vic: ".$query, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
	...
	if ($i1 && $i2 && $i3) {
		...
		if(isset($_POST['save-doublebay-vr2'])){
...
function generateRowItem2($section){ //type = double bay vr2, double bay vr3, double bay vr2 - gutter
	...
	if(isset($_POST['framework'])){
		...
		$framework = "Double Bay VR2";//set default value.
	}
	
	
	if($section=="frame"){
		$filter_framework = "";
	 	if($framework == "Double Bay VR2"){
		...
		while ($r = mysql_fetch_assoc($framework)) {
					...
					if( (strtolower($item["category"])=="beams" || strtolower($item["category"])=="posts" || strtolower($item["category"])=="beam fixings" || strtolower($item["category"])=="intermediate")){ 	
						...
						if(strtolower($item["category"]) == "beams" && $isFirst ==1 ){ 
							...
							if(METRIC_SYSTEM=="inch"){
								$tr .= "<td class=\"td-ft\"><input type=\"text\" id=\"cbeam_length_ft\" name=\"\" class=\"input-size \" value=\"\"></td>";  
							}
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items  WHERE LOWER(framework) = 'Double Bay VR2' AND LOWER(section) = 'fixings' {$order_by} " ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	// 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR3 - Guttering' AND LOWER(section) = 'guttering'" ); // 
			// $isFirst = 1;$isSecond = 0;$isThird = 0;$isFourth = 0;//should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
	 	// }else{
 		$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'guttering'" ); // 
		$isFirst = 1;$isSecond = 0;$isThird = 0;$isFourth = 0;//should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
	 	//}
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	//return; 
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'flashings' ORDER BY field(inventoryid, 'IRV43','IRV44','IRV280','IRV45','IRV46','IRV47') " );  
		$isFirst = 1;
		while ($r = mysql_fetch_assoc($framework)) { 
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  {$inventory_table} AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
			$tr .= "<tr>"; 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'downpipe'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'vergola' ORDER BY FIELD(inventoryid, 'IRV64','IRV63','IRV62','IRV60','IRV61','IRV59','IRV58','IRV54') DESC " ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'misc'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'extras'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR2' AND LOWER(section) = 'disbursements'" ); // 
...
// function listItem2($framework_type,$item=null,$itemId=""){
// 	$tag = "";  
// 	$framework_type = strtolower($framework_type);

// 	if($framework_type=="beams" || $framework_type=="post" || $framework_type=="intermediate"){ //type of item['category']
	 	
//  		$result = mysql_query("SELECT * FROM  {$inventory_table}  WHERE  LOWER(section) = 'frame' AND (LOWER(category) = 'beams' OR LOWER(category) = 'intermediate')" ); // include the category intermediate bec. it is in the middle of the 2 beam in a double bay.
// 		$tag .= "<select class=\"desclist\" name=\"desclist[]\" name=\"desclist[]\" id=\"{$itemId}\" >";
...
//  		//error_log("SELECT * FROM  {$inventory_table} WHERE  LOWER(section) = '{$framework_type}' AND category = 'Gutters Standard'", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
// 		//error_log($tag, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
...
// End of Table
echo "</table>";
//error_log("FINAL output vr2: ".$framework, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
?>
