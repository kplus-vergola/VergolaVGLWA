...
if(isset($_POST['save-doublebay-vr5']) || isset($_POST['save-close-doublebay-vr5'])){	
	...
	if ($i1 and $i2 and $i3) {
		...
		if(isset($_POST['save-close-doublebay-vr5'])){
...
function generateRowItemVR5($section, $fw=null){ //type = double bay vr2, double bay vr3, double bay vr2 - gutter
	...
	if($section=="frame"){
	 	...
	 	$framework = mysql_query("SELECT  * FROM ver_vergola_default_framework_items WHERE framework = 'Three Bay VR5'  AND (LOWER(section) = 'frame' OR LOWER(section) = 'posts') ORDER BY FIELD(inventoryid, 'IRV122','IRV25','IRV23','IRV15','IRV109','IRV111','IRV120','IRV3') DESC  " );  //ORDER BY FIELD(inventoryid, 'IRV122','IRV25','IRV23','IRV15','IRV111','IRV3') DESC  
		...
		while ($r = mysql_fetch_assoc($framework)) {
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items  WHERE LOWER(framework) = 'Three Bay VR5' AND LOWER(section) = 'fixings' {$order_by}" );  
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	if($fw == "Three Bay VR5"){ 
	 		$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'guttering' ORDER BY FIELD(inventoryid, 'IRV27','IRV28', 'IRV29', 'IRV30', 'IRV31')" ); // 
	 		//$isFirst = 1;$isSecond = 0;$isThird = 0;$isFourth = 0;//should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
	 	
	 	}else{ 
	 		//error_log("NOT INSIDE VR3 - gutter - section:".$section, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log'); 
	 		//error_log("INSIDE VR3 - gutter - framework:".$fw, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log'); 
	 		$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5 - Gutter' AND LOWER(section) = 'guttering' " ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) {
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'flashings' " ); // ORDER BY field(inventoryid,'IRV47','IRV46', 'IRV45', 'IRV280', 'IRV44', 'IRV43') desc
		...
		while ($r = mysql_fetch_assoc($framework)) { 
			...
			else if($isThird2==1){$lw = "width"; $isThird2=0;  }
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'downpipe'" ); 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'vergola' ORDER BY FIELD(inventoryid, 'IRV64','IRV63','IRV62','IRV60','IRV61','IRV59','IRV58','IRV54') DESC" );  
		
		$isFirst = 1; $isSecond = 0; $isSecond = 0;$isThird = 0;$isFourth = 0; $isFifth = 0; $isFifth2 = 0; $isSixth=0;$isSeventh=0;  $isEighth=0;
		...
		$tr .= "<td class=\"td-rrp\"> </td>";   
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'misc'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'extras'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR5' AND LOWER(section) = 'disbursements'" ); // 
...
if(isset($_POST['framework'])){
	...
	$framework = "Three Bay VR5";//set default value.
}
...
echo generateRowItemVR5("fixings"); 
...
echo generateRowItemVR5("guttering",$framework); 
 

// ***************************** Cbeam Face Flashing Z al ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Flashing</td></tr>"; 
echo generateRowItemVR5("flashings"); 
...
echo generateRowItemVR5("extras"); 
 
// ***************************** Shop Drawings ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Disbursements</td></tr>";
 echo generateRowItemVR5("disbursements"); 
