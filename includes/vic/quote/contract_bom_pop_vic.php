...
{ 
	...
	if($data != false){ 
		...
		$sql = "SELECT * FROM ver_chronoforms_data_inventory_vic WHERE inventoryid ='{$dimension['inventoryid']}'";
		//echo $sql;exit;
		$result = mysql_query($sql);
		...
		$html = "<form method='post' action='".JURI::base()."contract-listing-vic/contract-folder-vic/contract-bom-vic' >
				...
				<div class='item-dimension-content' style='width:700px;'>
					<div class='content-left'> <img src='".JURI::base()."images/inventory/{$item['photo']}"."' class='inv-image' /> </div>
					<div class='content-right'> 
						<p class='title'><h3>{$item['description']}</h3><span>Dimensions in mm</span></p>
...
{ 	 
	...
	if($is_error == false){ 
		...
		// $rrp = 0; $cost = 0; 
		// while ($paint = mysql_fetch_assoc($paints)) 
		// {
		// 	if($finish == $paint){ 
		// 		$rrp += ($length*$paint['rrp']);
		// 		$cost += ($length*$paint['rrp']);  
		// 	}
		// }
		...
		for ($i=0; $i<$cnt; $i++) {  
			...
			$vals = "(
							...
							if(isset($_POST['is_reorder'][$i])){
								$vals .= ",".mysql_real_escape_string($_POST['is_reorder'][$i]);
							}else{
								$vals .= ",0";
							}
							$vals .= ",'".mysql_real_escape_string($_POST['section'][$i])."'";
							$vals .= ",'".mysql_real_escape_string($_POST['category'][$i])."'";
				...
				$qry1 = "INSERT INTO ver_chronoforms_data_contract_bom_meterial_vic(projectid, inventoryid, materialid, raw_cost, qty, length, supplierid) 
						 	   (SELECT  '{$projectid}', im.inventoryid, im.materialid, m.raw_cost, " . mysql_real_escape_string($_POST['qty'][$i]) . ",".mysql_real_escape_string($_POST['length'][$i]).", m.supplierid 
 				...
 				//error_log(print_r($qry1,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
			 	mysql_query($qry1); 

		}
		//error_log(print_r($insertArr,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');//exit;
		$query = "INSERT INTO ver_chronoforms_data_contract_bom_vic (orderdate, quoteid, projectid, framework_type, framework, description, inventoryid, colour, finish, uom, qty, length, cost, rrp, supplierid, contract_item_cf_id, is_reorder, inventory_section, inventory_category) VALUES " . implode(", ", $insertArr);
	 	...
	 	$quoteid = mysql_real_escape_string($_POST['quoteid'][0]);
		$projectid = mysql_real_escape_string($_POST['projectid'][0]); 
	 	//header('Location: /contract-listing-vic/contract-po-vic?quoteid='.$quoteid.'&projectid='.$projectid);
...
if(isset($_POST['update_products']) && isset($_POST['is_modified']) && $_POST['is_modified']>0 ){
	...
	//print_r($_POST);return;
	    ...
	    for ($i=0; $i<count($_POST['inventoryid']); $i++) { 
	    			...
	    			error_log($inventoryid.'-'.$colour, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
	    // 			if($webbing=="Yes"){
	    ...
	    // 			$sql = "SELECT cf_id, rrp, cost, category FROM ver_chronoforms_data_inventory_vic WHERE section = 'Frame' and  cf_id  = '13' or cf_id = '14' or section ='Guttering' and cf_id = '41' or cf_id = '42'";	
	    			...
	    			$sql = "UPDATE ver_chronoforms_data_contract_items_vic SET inventoryid='$inventoryid', qty={$qty}, length={$length}, webbing='{$webbing}', colour='{$colour}', finish='{$finish}', rrp={$rrp}, cost={$cost}, supplierid='{$supplierid}' WHERE  cf_id={$cf_id}  ";
					//error_log($sql."- ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
					mysql_query($sql);

					$sql = "UPDATE ver_chronoforms_data_material_supplier_vic SET supplierid='{$supplierid}' WHERE projectid ='{$projectid}' AND inventoryid='$inventoryid'  ";
					//error_log($sql."- ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
					mysql_query($sql);

					//error_log($inventoryid."- ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
					//break;
					//exit();
	    		//}
	    	//} 
		...
		//$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  ver_chronoforms_data_inventory_vic AS i  WHERE inventoryid='".mysql_real_escape_string($_POST['inventoryid'][$i])."' " )); 

		$qry = mysql_query("SELECT * FROM ver_chronoforms_data_contract_items_vic WHERE quoteid = '$quoteid' and projectid = '$projectid'");
		//$contractItems = mysql_fetch_assoc($qResult);

		while ($cItem = mysql_fetch_assoc($qry)){

			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  ver_chronoforms_data_inventory_vic AS i  WHERE inventoryid='".$cItem['inventoryid']."' " )); 
			$total_rrp += $item['rrp'] * $cItem['qty'];	 

			//error_log(" total_rrp: ".$total_rrp, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 

			if($item['section'] == "Disbursements"){
				$total_disbursement += $item['rrp'];
			}
		}
		...
		//Compute payment
		$payment_deposit = 2000;
		$payment_progress = $total_sum * 0.65;
		$payment_final = $total_sum - $payment_deposit - $payment_progress;
 
		$com_sales_commission = $total_vergola * 0.1;
		$com_sales_commission_ps = 0;
		$com_pay1 = $com_sales_commission * 0.4;
		$com_pay2 = $com_sales_commission * 0.3;
		$com_final = $com_sales_commission * 0.3;
		$com_installer_payment = $total_vergola * 0.13;
	...
	if ($cnt > 0) {
		...
		for ($i=0; $i<$cnt; $i++) {     
			...
			$query = "DELETE FROM ver_chronoforms_data_contract_bom_vic  WHERE contract_item_cf_id={$cf_id}";
	 		...
	 		$query2 = "DELETE FROM ver_chronoforms_data_contract_bom_meterial_vic  WHERE projectid='{$projectid}' AND inventoryid='{$inventoryid}'  ";
	 	 	//echo $query;return;
...
if(isset($_POST['save_dimension'])){
	...
	//if($_POST['section'] == "Guttering"){
		...
		$query = "UPDATE ver_chronoforms_data_contract_items_deminsions  SET length='{$length}',  dimension_a='{$dimension_a}', dimension_b='{$dimension_b}', dimension_c='{$dimension_c}', dimension_d='{$dimension_d}', dimension_e='{$dimension_e}', dimension_f='{$dimension_f}', dimension_g='{$dimension_g}', dimension_h='{$dimension_h}', dimension_p='{$dimension_p}' WHERE cf_id='{$cf_id}';";
		//echo $query; exit();
		mysql_query($query) or trigger_error("Update failed: " . mysql_error()); 
...
function products($name,$inventory_id,$product_category="",$cat=""){
	$r = "<select class=\"sel-products\" name=\"{$name}\"  >"; 
...
<?php if(substr($projectid,0,3)=="VIC" || substr($projectid,0,3)=="JID" || substr($projectid,0,3)=="VSA" || substr($projectid,0,3)=="QID"){ ?>
	...
	$sqlquotes = "
		...
		s.company_name 
	FROM
		ver_chronoforms_data_contract_items_vic AS i
		LEFT JOIN ver_chronoforms_data_inventory_vic_old_system AS inv ON inv.inventoryid = i.inventoryid
		LEFT JOIN ver_chronoforms_data_inventory_vic AS inv1 ON inv1.inventoryid = i.inventoryid
		LEFT JOIN ver_chronoforms_data_supplier_vic AS s ON s.sup_id = i.supplierid 
	WHERE
		projectid = '$projectid' ";
...
<?php
    ...
    $project_framework = "";
    //------------- Framework Layout ---------------
    $sqlquotes = "SELECT i.*,i.cf_id AS id, inv.cf_id AS inv_cf_id, i.inventoryid AS inventory_id, inv.inventoryid, inv.section, inv.category, inv.description, inv.photo, inv.uom FROM ver_chronoforms_data_contract_items_vic AS i  LEFT JOIN ver_chronoforms_data_inventory_vic AS inv ON inv.inventoryid=i.inventoryid   WHERE projectid = '$projectid' and qty != 0 AND is_reorder=0 ORDER BY i.cf_id"; //ORGINAL QUERY  FIELD(i.inventoryid, 'IRV64','IRV63','IRV62','IRV60','IRV61','IRV59','IRV58','IRV54',     'IRV122','IRV121','IRV26','IRV25','IRV24','IRV23','IRV15','IRV120','IRV3') DESC 
	...
	if($is_error && count($error_messages)){
 	...
 	<form method="post"  >
 		...
 		<div class="" style="display:inline-block;">		
  		...
  		<?php echo "<a href=\"".JURI::base()."contract-listing-vic/contract-folder-vic/contract-po-vic?quoteid=".$quoteid."&projectid=".$projectid."\" class='btn '>&nbsp;&nbsp; View PO &nbsp;&nbsp;</a>"; ?>
  	</div>	
    ...
    <thead>
    	<tr>
    		<td colspan="8" class="subheading" data-section='Frame' >
				...
				<a href="javascript:void(0);" class="btn hide" onclick="$(this).closest('form').children('table').children('tbody').children('tr').children('td').children('select').prop('disabled', false);   " >test</a>
		...
		while($row = mysql_fetch_assoc($resultquotes)){
			...
			if($row["section"]=="Frame" || $row["section"]=="Posts"){
				...
				if($row["section"]=="Frame" && ($row["category"]=="Posts" || $row["category"]=="Beams" || $row["category"]=="Intermediate" || $row["category"]=="Beam Fixings")){
					echo "<td> ".products("inventoryid[]",$row['inventoryid'],'framework', $row["category"])."  </td>";   
				}else{
					echo "<td> ".$row['description']." <input type='hidden' name='inventoryid[]' value='{$row['inventoryid']}' category='{$row['category']}' /> </td>";  
				}
				
				echo "<td  class=\"td-item\"> {$row["uom"]} <input type=\"hidden\" class=\"price\" value=\"{$row['rrp']}\" category=\"{$row['category']}\"  inventoryid=\"{$row['inventoryid']}\" /> </td>"; 
