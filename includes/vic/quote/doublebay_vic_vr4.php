...
// 	 error_log("incorrect vr2: ".$_POST['framework'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
// 	 //$framework = "Double Bay VR2"; //set default value for get default item in database.
// 	//return;
// }else{
// 	error_log("correct vr2: ".$_POST['framework'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
// }  
...
if(isset($_POST['save-doublebay-vr4']) || isset($_POST['save-close-doublebay-vr4'])){	
	...
	if ($cnt6 > 0) { // && $cnt == $cnt2 && $cnt2 == $cnt3 && $cnt3 == $cnt4 && $cnt4 == $cnt5 && $cnt5 == $cnt6 && $cnt6 == $cnt7 && $cnt7 == $cnt8 && $cnt8 == $cnt9
		...
		//echo $queryn; return;
		//error_log($query, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');  //exit(); 
	...
	if ($cnt > 0) {
		...
		//error_log("INSERT INTO ver_chronoforms_data_measurement_vic: ".$query, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log'); 
...
function generateRowItem2($section){ //type = double bay vr2, double bay vr3, double bay vr2 - gutter
	...
	$tr = "";
	//$framework = $_POST['framework']; 
	//error_log("Inside generateRowItem2 framework: ".$_POST['framework'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
	if(isset($_POST['framework'])){
		$framework = $_POST['framework'];
	}else{
		$framework = "Three Bay VR4";//set default value.
	}
	
	
	if($section=="frame"){
		$filter_framework = "";
	 	if($framework == "Three Bay VR4"){
	 		$filter_framework = $framework;
	 	}  
	 	//error_log("Inside generateRowItem2 framework: ".$framework, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
	 	//error_log("FRAMEWORK: ".$framework, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
		...
		//error_log("SELECT  * FROM ver_vergola_default_framework_items WHERE framework = '{$filter_framework}'  AND (LOWER(section) = 'frame' OR LOWER(section) = 'posts') ORDER BY FIELD(inventoryid, 'IRV122','IRV25','IRV23','IRV15','IRV109','IRV111','IRV120','IRV3') DESC ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
		...
		while ($r = mysql_fetch_assoc($framework)) {
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items  WHERE LOWER(framework) = 'Three Bay VR4' AND LOWER(section) = 'fixings' {$order_by} " ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	// 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR3 - Guttering' AND LOWER(section) = 'guttering'" ); // 
			// $isFirst = 1;$isSecond = 0;$isThird = 0;$isFourth = 0;//should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
	 	// }else{
 		$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'guttering' ORDER BY FIELD(inventoryid, 'IRV31','IRV30','IRV29','IRV28','IRV27') DESC" ); // 
		$isFirst = 1;$isSecond = 0; $isSecond2 = 0;$isThird = 0;$isFourth = 0; $isFourth2 = 0; $isFifth=0; $isSixth=0; //should be in order based on the list from the pulled record from database, beam(L), beam(L[2nd]), 
	 	//}
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	//return; 
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'flashings' ORDER BY field(inventoryid, 'IRV43','IRV44','IRV280','IRV45','IRV46','IRV47') " );  
		$isFirst = 1;
		while ($r = mysql_fetch_assoc($framework)) { 
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  {$inventory_table} AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
			$tr .= "<tr>"; 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'downpipe'" ); 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'vergola' ORDER BY FIELD(inventoryid, 'IRV64','IRV63','IRV62','IRV60','IRV61','IRV59','IRV58','IRV54') DESC " ); 
		...
		while ($r = mysql_fetch_assoc($framework)){
			...
			if($isFirst || $isSecond || $isSecond2){
				$itemName = listItem("louvres",null,null,"poly".($isFirst?"1":"").($isSecond?"2":"").($isSecond2?"3":""));
			...
			$tr .= "<tr class=\"tr-vergola\">"; 
				...
				if($item["inventoryid"] == "IRV61"){
					...
					$tr .= "<td class=\"td-qty\"><input   type=\"text\" {$itemQty} class=\"qtylen ".(($isFirst||$isSecond||$isSecond2)?"louver-item-qty":"")."\" name=\"qty[]\" value=\"{$r['qty']}\"></td>";  
				}
		...
		$tr .= "<td class=\"td-rrp\"> </td>";   
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'misc'" );  
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'extras'" ); // 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Three Bay VR4' AND LOWER(section) = 'disbursements'" ); // 
...
// End of Table
echo "</table>"; 
//error_log("FINAL output vr2: ".$framework, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_vic\\my-error.log');
?>
