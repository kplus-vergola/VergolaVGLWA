...
if(isset($_POST['save-doublebay-vr3-gutter']) || isset($_POST['save-close-doublebay-vr3-gutter']) ){	
	...
	if ($i1 and $i2 and $i3) {
		...
		if(isset($_POST['save-close-doublebay-vr3-gutter']) ){    
...
function generateRowItemVR3G($section, $fw=null){ //type = double bay vr2, double bay vr3, double bay vr2 - gutter
	global $inventory_table;
	$tr = "";
	//error_log("HERE A", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
	if($section=="frame"){
	 	...
	 	$framework = mysql_query("SELECT  * FROM ver_vergola_default_framework_items WHERE framework = 'Double Bay VR3'  AND (LOWER(section) = 'frame' OR LOWER(section) = 'posts') ORDER BY FIELD(inventoryid, 'IRV122','IRV25','IRV23','IRV15','IRV109','IRV111','IRV120','IRV3') DESC " ); //  
		...
		//error_log(print_r($framework,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
			...
			$tr .= "<tr>"; 
					...
					if(  (strtolower($item["category"]) == "beams" || strtolower($item["category"])=="posts" || strtolower($item["category"])=="beam fixings" || strtolower($item["category"])=="intermediate")  ){ //
						...
						if(strtolower($item["category"]) == "beams" && $isFirst ==1 ){ 
							...
							//$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"input-size\" value=\"\"></td>";  
							$isFirst = 0;
						...
						//error_log("2st TR: ".$tr, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
					}else{
		...
		while ($r = mysql_fetch_assoc($framework)) {
			...
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  {$inventory_table} AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
			//error_log(" Item: ".$item['inventoryid'] ."-".$r['qty'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); 
			$tr .= "<tr>"; 
					...
					if($item["inventoryid"] == "" && $isSecond==1){ 
						...
						if($_isFirst==1){
							$tr .= "<td class=\"td-len\"><input type=\"text\"   name=\"slength[]\" class=\"length input-in gutter-length\" value=\"0\"></td>";  //class = tapered_gutter_IRV33_34_length
							if(METRIC_SYSTEM=="inch"){
								$tr .= "<td class=\"td-ft\"><input type=\"text\"  class=\"length input-ft gutter-length\" value=\"0\"></td>";
							}
							$_isFirst = 0;
						}else{
							$tr .= "<td class=\"td-len\"><input type=\"text\"   name=\"slength[]\" class=\"length2 input-in gutter-length\" value=\"0\"></td>";  //class = tapered_gutter_IRV33_34_length
							if(METRIC_SYSTEM=="inch"){
						...
						if(METRIC_SYSTEM=="inch"){
							$tr .= "<td class=\"td-ft\"><input type=\"text\"  id=\"gutterLiningLength_ft\"   class=\" input-in\" value=\"0\"></td>"; 
						}
			...
			$tr .= "<tr>"; 
					...
					if($isSecond == 1 || $isThird == 1 || $isFourth == 1){ //($item["inventoryid"]=="IRV43" || $item["inventoryid"]=="IRV46" )  && 
						...
						if($isFirst_IRV280){
							...
							if(METRIC_SYSTEM=="inch"){
								$tr .= "<td class=\"td-ft\"><input id=\"IRV280_l1_ft\" type=\"text\"   class=\" input-ft\" value=\"\"></td>";
							}
					 		...
					 		if(METRIC_SYSTEM=="inch"){
					 			$tr .= "<td class=\"td-ft\"><input id=\"IRV280_l2_ft\" type=\"text\"  class=\" input-ft\" value=\"\"></td>";
							}				 
						...
						if(METRIC_SYSTEM=="inch"){
							$tr .= "<td class=\"td-ft\"><input type=\"text\"  name=\"slength[]\" class=\"width input-in\" value=\"0\" id=\"{$input_id}_ft\"></td>";
						}						 
		...
		while ($r = mysql_fetch_assoc($framework)) { 
	 	...
	 	$framework = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Double Bay VR3' AND LOWER(section) = 'vergola' ORDER BY FIELD(inventoryid, 'IRV54','IRV58', 'IRV59', 'IRV61', 'IRV60', 'IRV62', 'IRV63', 'IRV64', 'IRV66' ) " ); // ORDER BY FIELD(inventoryid, 'IRV54','IRV58', 'IRV59', 'IRV60', 'IRV62', 'IRV63', 'IRV64', 'IRV61', 'IRV54', 'IRV59')
		...
		while ($r = mysql_fetch_assoc($framework)) { 
			...
			if($isFirst || $isThird){
				$itemName = listItem("louvres",null,null,"poly".($isFirst?"1":"2"));
	...
	//error_log("Return: ".$tr, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log');
	return $tr;
...
echo generateRowItemVR3G("frame"); 
...
echo generateRowItemVR3G("fixings"); 
...
echo generateRowItemVR3G("extras"); 
 
// ***************************** Shop Drawings ********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Disbursements</td></tr>";
 echo generateRowItemVR3G("disbursements"); 
