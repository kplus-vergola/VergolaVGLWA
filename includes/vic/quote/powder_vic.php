...
while ($paint = mysql_fetch_array($resultpp)) {
		...
		if ($paint['cf_id'] == '14'){echo "selected=\"selected\"";}
		echo ">".$paint['category']."</option>";	
		}
echo "</select>";
$datapaint = mysql_query("SELECT cf_id, rrp, cost, category FROM ver_chronoforms_data_inventory_vic WHERE section = 'Frame' and category  = 'Powder'" );
