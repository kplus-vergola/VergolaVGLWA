...
$resultgutter = mysql_query("SELECT cf_id, inventoryid, description, uom, rrp, cost FROM ver_chronoforms_data_inventory_vic WHERE section = 'Guttering' and category  = 'Gutters Non Standard'" );
        ...
        $BeamUOMArrayPhp .= isset($gutter[3]) ? 'BeamUOMArray["'.$heading.'"]="'.$gutter[3].'";' : null;
      	echo "<option value=\"".$gutter['cf_id']."\"";
		if($gutter['cf_id'] == '32') { echo "selected=\"selected\""; } echo ">".$gutter['description']."</option>";	
		}	

echo "</select>";
    
	 $guttering = mysql_query("SELECT cf_id, inventoryid, description, uom, rrp, cost FROM ver_chronoforms_data_inventory_vic WHERE section = 'Guttering' and category  = 'Gutters Non Standard' and cf_id = '32'" );
