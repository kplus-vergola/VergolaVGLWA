...
while ($paint = mysql_fetch_array($resultpp)) {
		...
		$BeamCOSTArrayPhp .= isset($paint[2]) ? 'BeamCOSTArray["'.$heading.'"]="'.$paint[2].'";' : null;
      	echo "<option value=\"".$paint['cf_id']."\">".$paint['category']."</option>";	
		}
echo "</select>";
$datapaint = mysql_query("SELECT cf_id, rrp, cost, category FROM ver_chronoforms_data_inventory_vic WHERE section = 'Frame' and category  = 'Paint'" );
