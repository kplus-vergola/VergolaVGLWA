...
{	
	...
	$DateLodged = date('Y-m-d H:i:s'); //$DateLodged;
	...
	if($section=="frame"){
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			if(METRIC_SYSTEM=="inch"){
				if($item["inventoryid"] == "IRV31"){
					...
					$tr .= "<td class=\"td-ft\"><input type=\"text\"  class=\"gutter-length input-ft\" value=\"0\"  ></td>";  
				}	
	 	...
	 	$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Single Bay VR1' AND LOWER(section) = 'flashings' AND inventoryid NOT IN ('IRV48') ORDER BY  FIELD(inventoryid,'IRV46','IRV46','IRV45','IRV280','IRV44','IRV43') DESC " ); //IRV48 was excluded in the code bec. it is just used in VR0. VR1 and VR0 is just using almost whole code and using this part of code expect excluding IRV48. 
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<tr>"; 
					...
					}elseif(($item["inventoryid"]=="IRV43" || $item["inventoryid"]=="IRV46" ) || ($item["inventoryid"]=="IRV44" || $item["inventoryid"]=="IRV280")){ //error_log("2nd", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_us\\my-error.log'); IRV280
						$tr .= "<td class=\"td-len\"><input type=\"text\"  name=\"slength[]\" class=\"width input-in\" value=\"0\"></td>";	 
	 	...
	 	$fw = mysql_query("SELECT * FROM ver_vergola_default_framework_items   WHERE  framework = 'Single Bay VR1' AND LOWER(section) = 'downpipe' AND inventoryid NOT IN ('IRV51')" ); // 
		...
		while ($r = mysql_fetch_assoc($fw)) { 
			...
			$tr .= "<td class=\"td-qty\"><input type=\"text\" {$itemQty} class=\"{$className}\" name=\"qty[]\" value=\"{$r['qty']}\"></td>";  
