...
if(isset($_POST['save-multiplebay']))
{	
...
$cnt7 = count($_POST['rrp']);
$cnt8 = count($_POST['cst']);


if ($cnt > 0 ) { //&& $cnt == $cnt2 && $cnt2 == $cnt3 && $cnt3 == $cnt4 && $cnt4 == $cnt5 && $cnt5 == $cnt6 && $cnt6 == $cnt7 && $cnt7 == $cnt8
    $insertArr = array();
    
	for ($i=0; $i<$cnt; $i++) { 
		$insertArr[] = "('$QuoteID', '" . mysql_real_escape_string($_POST['invent'][$i]) . "', '$getprojectid', '$ProjectName', '$FrameworkTYP', '$Framework', '" . mysql_real_escape_string($_POST['desc'][$i]) . "', '" . mysql_real_escape_string($_POST['colour'][$i]) . "', '" . mysql_real_escape_string($_POST['qty'][$i]) . "', '" . mysql_real_escape_string($_POST['webbing'][$i]) . "', '" . mysql_real_escape_string($_POST['paint'][$i]) . "', '" . mysql_real_escape_string($_POST['uom'][$i]) . "', '" . mysql_real_escape_string($_POST['rrp'][$i]) . "', '" . mysql_real_escape_string($_POST['cst'][$i]) . "')";
	}

	$queryn = "INSERT INTO ver_chronoforms_data_quote_vic (quoteid, inventoryid, projectid, project_name, framework_type, framework, description, colour, qty, webbing, finish, uom, rrp, cost) VALUES " . implode(", ", $insertArr);
	...
	$num_bay = $_POST['bay'];
	$insertArr = array();
	//error_log("BAY : ".$num_bay."     ", 3,'C:\\xampp\htdocs\\vergola_contract_system_v3\\my-error.log'); 
 	for ($i=0; $i<$num_bay; $i++) { 
		$insertArr[] = "('$getprojectid', '$FrameworkTYP', '" . mysql_real_escape_string($_POST['width']) . "', '" . mysql_real_escape_string($_POST['length']) . "', '" . mysql_real_escape_string($_POST['bay']) . "')";
	}
	...
	//error_log($querym, 3,'C:\\xampp\htdocs\\vergola_contract_system_v3\\my-error.log'); 
...
$Totalrrp = $_POST['subtotalvergola'];
$Totalrrpgst = $_POST['totalrrpgst'];
$Totalcost = $_POST['totalcost'];
$Totalcostgst = $_POST['totalcostgst'];

$queryp = "INSERT INTO ver_chronoforms_data_followup_vic (sales_rep, quoteid, projectid, quotedate, project_name, framework_type, total_rrp, total_rrp_gst, total_cost, total_cost_gst, sales_comm, install_comm, sales_comm_cost, install_comm_cost, status) VALUES ('$salesrep', '$QuoteID', '$getprojectid', '$DateLodged', '$ProjectName', '$FrameworkTYP', '$Totalrrp', '$Totalrrpgst', '$Totalcost', '$Totalcostgst', '$SalesComm', '$InstallComm', '$SalesCost', '$InstallCost', 'Quoted')";
...
if(isset($_POST['save-close-multiplebay']))
{	
$ProjectName = $_POST['projectsite'];
$DateQuote = $_POST['quotedate'];
$FrameworkTYP = $_POST['frameworktype'];
...
echo "<tr><th>Description</th><th>Webbing</th><th>Colour</th><th>Finish</th><th>OUM</th><th>QTY</th><th>Length</th><th>RRP</th></tr>";


// ***************************** Cbeam 200 Deep by 2.4mm ***********************************************//
echo "<tr><td colspan=\"8\" class=\"subheading\">Framework</td></tr>";
$resultcbeam200 = mysql_query("SELECT cf_id, inventoryid, description, uom, rrp, cost FROM ver_chronoforms_data_inventory_vic WHERE section = 'Frame' and category = 'CBeams' or category = 'Beams' or category = 'Timber'" );
...
$resultcbeam250 = mysql_query("SELECT cf_id, inventoryid, description, uom, rrp, cost FROM ver_chronoforms_data_inventory_vic WHERE section = 'Frame'  and (category = 'CBeams' or category = 'Beams' or category = 'Timber')" );
echo "<tr><td><select class=\"desclist\" name=\"desclist[]\" >";
...
echo "</select>";
	 ...
	 $datacbeam250 = mysql_query("SELECT cf_id, inventoryid, description, uom, rrp, cost FROM ver_chronoforms_data_inventory_vic WHERE  cf_id = '3'" );
