...
function pagination($current_page_number, $total_records_found, $query_string = null)
{
	...
	for ($total_pages = ($total_records_found/NUMBER_PER_PAGE); $total_pages > 0; $total_pages--)
	{
		if ($page != $current_page_number)
			echo "<a href=\"" . "system-management-vic/installer-listing-vic" . "?page=$page" . (($query_string) ? "&$query_string" : "") . "\">";
...
<input type='button' class='add-btn' onclick=location.href='" . JURI::base() . "system-management-vic/installer-listing-vic/add-installer?view=add' value='Add New'>
</div>";
...
$sql = "SELECT * FROM ver_chronoforms_data_installer_vic WHERE 1=1 AND status != 'deleted'";
...
$loop = mysql_query($sql)
	...
	echo "<table class='listing-table table-bordered'><thead><tr><th>Installer Name</th><th>Mobile</th><th>Email</th> </tr></thead><tbody>";
while ($record = mysql_fetch_assoc($loop))
    echo "<tr class='pointer' onclick=location.href='" . $this->baseurl . "installer-listing-vic/add-installer?view=edit&cf_id={$record['cf_id']}' ><td>{$record['name']}</td><td>{$record['mobile']}</td><td>{$record['email']}</td></tr>";
    echo "</tbody></table>"; 
