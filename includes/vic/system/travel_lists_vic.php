...
function pagination($current_page_number, $total_records_found, $query_string = null)
{
	...
	for ($total_pages = ($total_records_found/NUMBER_PER_PAGE); $total_pages > 0; $total_pages--)
	{
		if ($page != $current_page_number)
			echo "<a href=\"" . "system-management-vic/travel-listing-vic" . "?page=$page" . (($query_string) ? "&$query_string" : "") . "\">";
...
<input type='button' class='add-btn' onclick=location.href='" . JURI::base() . "system-management-vic/travel-listing-vic/travel-vic' value='Add New'>
</div>";
...
$sql = "SELECT * FROM ver_chronoforms_data_travel_vic WHERE 1=1 AND status != 'deleted'";
$result = mysql_query($sql) or die(mysql_error());

if ($search)
	$sql .= " AND (zone LIKE '%"  . $search .  "%'" . " OR cost LIKE '%"  . $search .  "%')";
...
pagination($page, $total_records, "zone=$zone&cost=$cost");
echo "</div>";

$loop = mysql_query($sql)
	or die ('cannot run the query because: ' . mysql_error());
	echo "<table class='listing-table table-bordered'><thead><tr><th>Zone</th><th>Cost</th></tr></thead><tbody>";
while ($record = mysql_fetch_assoc($loop))
    echo "<tr class='pointer' onclick=location.href='" . $this->baseurl . "travel-listing-vic/travel-updatelist-vic?cf_id={$record['cf_id']}' ><td>{$record['zone']}</td>" . "<td>{$record['cost']}</td></tr>";
    echo "</tbody></table>"; 
