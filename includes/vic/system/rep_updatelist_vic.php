...
if (isset($_POST['view_sales_target'])) {
    ...
    // Build the new URL (include port if present)
...
<div style="width:500px; font-size:12px; float: left; position: relative; left: 50px;">
	...
	<table id="tblSalesTarget" class="update-table">
		<tr>
			...
			<?php
				...
				if(isset($_GET['dFrom']) && isset($_GET['dTo'])){
				    ...
				    $yearResult = mysql_query("SELECT MAX(year) AS latest_year FROM ver_rep_sales_target WHERE rep_id='$RepID'");
				...
				$formatTo = date_format(date_create($toDate->format('Y-m-d')), PHP_DFORMAT);
				?>
		...
		<?php 
			...
			$qResult = mysql_query("SELECT * FROM ver_rep_sales_target WHERE rep_id='$RepID' AND (year={$year} OR year={$year2})");
