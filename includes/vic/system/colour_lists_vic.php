...
function pagination($current_page_number, $total_records_found, $query_string = null)
{
	...
	for ($total_pages = ($total_records_found/NUMBER_PER_PAGE); $total_pages > 0; $total_pages--)
	{
		if ($page != $current_page_number)
			echo "<a href=\"" . "system-management-vic/colour-listing-vic" . "?page=$page" . (($query_string) ? "&$query_string" : "") . "\">";
...
<input type='button' class='add-btn' onclick=location.href='" . JURI::base() . "system-management-vic/colour-listing-vic/colour-vic' value='Add New'>
</div>";
...
$sql = "SELECT * FROM ver_chronoforms_data_colour_vic WHERE 1=1 AND status != 'deleted'";
$result = mysql_query($sql) or die(mysql_error());

if ($search)
	$sql .= " AND (colour LIKE '%"  . $search .  "%')";
...
pagination($page, $total_records, "colour=$colour");
echo "</div>";

$loop = mysql_query($sql)
	or die ('cannot run the query because: ' . mysql_error());
	echo "<table class='listing-table table-bordered'><thead><tr><th>Colour</th> </tr></thead><tbody>";
while ($record = mysql_fetch_assoc($loop))
    echo "<tr class='pointer' onclick=location.href='" . $this->baseurl . "colour-listing-vic/colour-updatelist-vic?cf_id={$record['cf_id']}' ><td>{$record['colour']}</td></tr>";
    echo "</tbody></table>"; 
