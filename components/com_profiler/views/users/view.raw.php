 ...
 * @version $Id: view.raw.php 48 2013-06-10 21:36:21Z harold $
 * @author Harold Prins
...
class ProfilerViewUsers extends JViewLegacy {
	...
	function display($tpl = null)
	{
		...
		if(!$ajax)
			$pagination	= $this->get('Unsetpagination');
				...
				if(($item || $r == "avatar") && isset($this->fields[$r])) {
					...
					switch ($field['type']) {
						...
						case "avatar":
							...
							if($item) {
								$value		= $item ? JURI::base().'media/com_profiler/'.$folder.'/'.$field['type'].'/'.$item : '';
							} else {
								$value		= JURI::base().'media/com_profiler/img/user.png';
							}
	...
	protected function _prepareFieldsExcel()
	{
		$config = JComponentHelper::getParams('com_profiler');
		foreach($this->items as &$row) {
			foreach($row as $r => &$item) {
				if($item && isset($this->fields[$r])) {
					$field = $this->fields[$r];
					switch ($field['type']) {
						case "calendar":
							$format = $field['format'] ? (string) $field['format'] : $config->get('datetimedisplay');
							if($item && $item!="0000-00-00 00:00:00" && $item!="0000-00-00"){
								jimport('joomla.utilities.date');
								$date = new JDate($item);
								$item = htmlspecialchars($date->format($format), ENT_COMPAT, 'UTF-8');
							} else {
						...
						case "list":
							if($field['multiple']) {
								$itemarray = json_decode($item, true);
								$item = "";
								//itemarray ( [0] = 10, [1] = ... enz)
								//$field['value'] = 1=die,2=die
								//afmaken
								if(is_array($itemarray)) {
									$values = explode(",", $field['value']);
									foreach($values as $value) {
										$valuedetail = explode("=", $value);
										if(in_array($valuedetail[0], $itemarray)) {
											if($item)
												$item .= ", ";
											$item .= $valuedetail[1];
										}
