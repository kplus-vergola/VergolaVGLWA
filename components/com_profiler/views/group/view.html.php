 ...
 * @version $Id: view.html.php 11 2013-03-09 15:33:47Z harold $
 * @author Harold Prins
...
class ProfilerViewGroup extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		if ((!$active) || ((strpos($active->link, 'view=user') === false) || (strpos($active->link, '&groupid=' . (string) $this->item->groupid) === false))) {
			if ($layout = $this->params->get('grouplist_layout')) {
		...
		JFactory::getApplication()->setUserState('com_profiler.edit.group.data', null);
	...
	public function loadPlugin($return = "all") {
		JPluginHelper::importPlugin('profiler');
		$dispatcher =& JDispatcher::getInstance();
		$results = $dispatcher->trigger('display', array($this->item->groupid));
		$return = array();
		if(is_array($results)) {
			foreach ($results as $result) {
